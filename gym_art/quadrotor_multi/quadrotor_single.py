...
class QuadrotorSingle:
    ...
    def make_observation_space(self):
        ...
        self.obs_space_low_high = {
            ...
            "dist": [np.array([0]), np.array([np.linalg.norm(self.room_box[1] - self.room_box[0])])],
            "rad": [np.array([self.dynamics.arm]), np.array([4 * self.dynamics.arm])],
        }
        ...
        elif self.swarm_obs == 'cadrl' and self.num_agents > 1:
            obs_comps = obs_comps + (['dist'] + ['rxyz'] + ['vxyz'] + ['dist'] + ['rad']) * self.num_use_neighbor_obs
